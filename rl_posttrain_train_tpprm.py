...
#   --grad_accum 2 \
#   --reward_mode prm_tp \
#   --verifier_model_name Qwen/Qwen2.5-Math-1.5B-Instruct \
#   --kl_beta 0.001 \
#   --temperature 0.7 \
#   --validation_size 64 \
...
class GenerativeMathVerifier:
    """
    Lightweight verifier interface for thought-level process scoring.

    Expected prompt behavior:
    - Given problem, current thought, and optionally previous thoughts, the verifier emits one token / short answer
      from {correct, incorrect, unsure}.
    - You should replace `score_thought` prompt details with your best math verifier prompt.

    If no verifier_model_name is provided, this falls back to symbolic heuristics only.
    """
    ...
    @torch.no_grad()
    def _generate_label(self, prompt: str) -> Optional[str]:
        ...
        txt = self.tokenizer.decode(new_ids, skip_special_tokens=True).strip().lower()
        return txt
    ...
    def score_thought(
        ...
        """
        Returns (score, label) where label in {correct, incorrect, unsure}.
        Score is in {-1, 0, +1} by default.
        """
        # heuristic shortcut for explicit equation lines if sympy is available
        ...
        if self.model is None:
            # fallback symbolic/weak heuristic
            if final_is_correct:
                return 1.0, "correct"
            if final_prediction is not None and gold_answer is not None and current_thought:
                return 0.0, "unsure"
            return 0.0, "unsure"

        prompt = (
            "You are a strict math process verifier.\n"
            "Judge whether the CURRENT_THOUGHT is mathematically correct with respect to solving the QUESTION.\n"
            "Use labels exactly: correct / incorrect / unsure.\n\n"
            f"QUESTION:\n{question}\n\n"
            f"PREVIOUS_THOUGHTS:\n{chr(10).join(previous_thoughts[-3:]) if previous_thoughts else '[none]'}\n\n"
            f"CURRENT_THOUGHT:\n{current_thought}\n\n"
            f"GOLD_FINAL_ANSWER:\n{gold_answer}\n\n"
            f"MODEL_FINAL_ANSWER:\n{final_prediction}\n\n"
            "Reply with one word only."
        )
        txt = self._generate_label(prompt)
        if txt is None:
            return 0.0, "unsure"
        if "incorrect" in txt:
            return -1.0, "incorrect"
...
def load_prm_if_needed():
    ...
    scalar_prm_params = 0
    verifier = GenerativeMathVerifier(CFG.verifier_model_name, CFG.verifier_device)
    ...
    if mode == "prm_scalar":
        ...
        scalar_prm_model = AutoModelForCausalLM.from_pretrained(
            CFG.prm_model_name,
            trust_remote_code=True,
