...
BASE_STEPS_REF_C1="${BASE_STEPS_REF_C1:-200}"
BASE_STEPS_REF_C2="${BASE_STEPS_REF_C2:-400}"
BASE_STEPS_REF_C3="${BASE_STEPS_REF_C3:-600}"

base_steps_for_band () {
  case "$1" in
    C1) echo "${BASE_STEPS_REF_C1}" ;;
    C2) echo "${BASE_STEPS_REF_C2}" ;;
    C3) echo "${BASE_STEPS_REF_C3}" ;;
    *)  echo "${BASE_STEPS_REF_C1}" ;;
  esac
}
...
BASE_STEPS_REF="$(base_steps_for_band "$BAND")"

STEPS_REF_ALLOC_RANK=$(python - <<PY
base=float("${BASE_STEPS_REF}")
amult=float("${AMULT}")
kmult=float("${KMULT}")
...
OUTDIR="runs_spot_alloc_scaling/${SCOPE}/${BAND}/${REWARD}/${ALLOC}/${MODEL_TAG}/r${LORA_RANK}/seed${SEED}/K${K_FIXED}_L${L_FIXED}_S${STEPS}"
