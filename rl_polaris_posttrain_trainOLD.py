...
# - preserve symbolic answers for Polaris / MATH-style data
# - no collapsing gold answers like "-\\frac{82}{9}" -> "9"
# - use a symbolic/numeric equivalence checker across ALL reward modes
# - sparse reward remains true binary 0/1
# - real KL regularization via GRPOConfig(beta=...)
# - logs raw completions and max-length hits
#
# Suggested sparse pilot:
# python rl_posttrain_train.py \
#   --model Qwen/Qwen2.5-1.5B-Instruct \
#   --out runs_sparse_kl_pilot \
#   --steps 100 \
#   --K 2 \
#   --L 256 \
#   --grad_accum 2 \
...
def _sympy_expr_from_text(s: str):
    ...
    # Keep known function names intact before letter-splitting
    placeholders = {
        "sqrt": "__SQRT__",
        "sin": "__SIN__",
        "cos": "__COS__",
    ...
    # Only add multiplication in clearly mathematical cases.
    # 2x -> 2*x
...
def _looks_symbolic_math(s: str) -> bool:
    ...
    if any(tok in s for tok in [
        ...
        return True
    # Also treat multi-letter algebraic strings as symbolic, e.g. ab, abc, xy
...
def canonical_answer(x: Optional[str]) -> Optional[str]:
    ...
    # If the whole thing is numeric, canonicalize numerically
    f = _safe_float(s)
    if f is not None:
...
def build_rewards_and_accounting(
    ...
    def extract_sparse_answer(text: str) -> Optional[str]:
        ...
        # only very conservative fallback: short final line
        lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
        if lines:
            last = lines[-1]
            if len(last) <= 80:
